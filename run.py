...
def train(hparams, checkpoint_callback, meta_data_vocab, train_dataloader, val_dataloader, test_dataloader,
          all_sentences):
    model = Model(hparams, meta_data_vocab)
    logger = get_logger('train', hparams)
    trainer = Trainer(show_progress_bar=True, num_sanity_val_steps=hparams.num_sanity_val_steps, gpus=hparams.gpus,
                      logger=logger, checkpoint_callback=checkpoint_callback, min_epochs=hparams.epochs,
                      max_epochs=hparams.epochs,
                      accumulate_grad_batches=int(hparams.accumulate_grad_batches),
                      gradient_clip_val=hparams.gradient_clip_val, num_tpu_cores=hparams.num_tpu_cores,
                      use_tpu=hparams.use_tpu, train_percent_check=hparams.train_percent_check,
...
def resume(hparams, checkpoint_callback, meta_data_vocab, train_dataloader, val_dataloader, test_dataloader,
           all_sentences):
    ...
    trainer = Trainer(show_progress_bar=True, num_sanity_val_steps=5, gpus=hparams.gpus, logger=logger,
                      checkpoint_callback=checkpoint_callback, min_epochs=hparams.epochs, max_epochs=hparams.epochs,
                      resume_from_checkpoint=checkpoint_path, accumulate_grad_batches=int(
            hparams.accumulate_grad_batches), gradient_clip_val=hparams.gradient_clip_val,
                      num_tpu_cores=hparams.num_tpu_cores, use_tpu=hparams.use_tpu,
                      train_percent_check=hparams.train_percent_check, track_grad_norm=hparams.track_grad_norm,
...
def test(hparams, checkpoint_callback, meta_data_vocab, train_dataloader, val_dataloader, test_dataloader,
...
def predict(hparams, checkpoint_callback, meta_data_vocab, train_dataloader, val_dataloader, test_dataloader,
            all_sentences, mapping=None, conj_word_mapping=None):
...
def splitpredict(hparams, checkpoint_callback, meta_data_vocab, train_dataloader, val_dataloader, test_dataloader,
                 all_sentences):
    ...
    split_test_dataloader = DataLoader(split_test_dataset, batch_size=hparams.batch_size, collate_fn=data.pad_data,
                                       num_workers=1)
    ...
    if hparams.rescoring:
        ...
        for line_i, line in enumerate(rescored):
            ...
            extraction = Extraction(pred=rel, head_pred_index=None, sent=sentence, confidence=math.exp(confidence),
                                    index=0)
...
def get_labels(hparams, model, sentences, orig_sentences, sentences_indices):
    ...
    for i in range(0, len(sentences_indices)):
        ...
        for j in range(0, len(sentences_indices[i])):
            assert len(sentences_indices[i][j]) == len(
                outputs[idx1]['meta_data'][idx2].strip().split()), ipdb.set_trace()
...
def main(hparams):
    ...
    if hparams.save != None:
        checkpoint_callback = ModelCheckpoint(
            filepath=hparams.save + '/{epoch:02d}_{eval_acc:.3f}', verbose=True, monitor='eval_acc', mode='max',
            save_top_k=hparams.save_k if not hparams.debug else 0, period=0)
    else:
